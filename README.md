...
| Virtual categories (multi-label) | ✅ tests pass; no real dirs ever created |
| Incremental re-index of changed files | ✅ fixed via path-stable ids (see below) |
| **Exact duplicate detection** | ⚠️ **KNOWN BROKEN** — `computeDuplicateGroups()` returns `[]` even for identical files; report-only by design so nothing is at risk. Do not trust it yet. |
...
| Sandboxed .app entitlement audit in CI | script exists (`scripts/audit_entitlements.py`), CI wiring pending |
