...
final class LiveIndexCoordinatorTests: XCTestCase {
    ...
    func testUserAndKernelDroppedAlsoTriggerRescan(){for flag in [LiveRawEvent.userDropped,LiveRawEvent.kernelDropped,LiveRawEvent.rootChanged]{let q=LiveCoalescingQueue(debounceInterval:0.8); q.ingest([LiveRawEvent(path:"/a/b",flags:flag,eventId:1)]); XCTAssertTrue(q.hasPendingFullRescan,"flag \(flag) must trigger rescan")}}
    func testHistoryDoneSentinelDoesNotForceFullRescan(){let q=LiveCoalescingQueue(debounceInterval:0.8); q.ingest([LiveRawEvent(path:"/",flags:LiveRawEvent.historyDone,eventId:1)]); XCTAssertFalse(q.hasPendingFullRescan,"the one-per-stream historyDone sentinel must not force a rescan of every root on every coordinator start")}
    ...
    func testDirectoryEventReconcilesChildren() throws {
        let tmp=URL(fileURLWithPath:"/private/tmp").appendingPathComponent("live-dir-event-\(UUID().uuidString)")
        let sub=tmp.appendingPathComponent("sub")
        try FileManager.default.createDirectory(at:sub,withIntermediateDirectories:true); defer{try? FileManager.default.removeItem(at:tmp)}
        let file=sub.appendingPathComponent("new.txt")
        try Data("new".utf8).write(to:file)
        let catalog=try makeCatalog(); let broker=SourceBroker()
        let indexer=Indexer(broker:broker,catalog:catalog,scheduler:Scheduler())
        let c=LiveIndexCoordinator(catalog:catalog,indexer:indexer,broker:broker,scheduler:Scheduler(),roots:[tmp])

        c.ingest(events:[LiveRawEvent(path:sub.path,flags:LiveRawEvent.itemModified|LiveRawEvent.itemIsDir,eventId:1)])
        _ = c.flushForTesting()

        XCTAssertTrue(try catalog.allFiles().contains { $0.path == file.path })
        XCTAssertFalse(try catalog.allFiles().contains { $0.path == sub.path })
    }
}
