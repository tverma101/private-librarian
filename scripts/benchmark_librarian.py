...
Metrics (all synthetic + local):
  ...
  model-worker warmup, wall time, files/sec, peak RSS where available,
  DB size, per-stage counts. Tier-1-only and Tier-2-enabled modes.
"""
...
def time_search_loop(binary: Path, catalog: Path, query: str, iters: int = SEARCH_ITERS) -> dict:
    env = dict(os.environ)
    env["LIBRARIAN_CATALOG_KEY"] = BENCHMARK_KEY_HEX
    ...
    for _ in range(iters):
        ...
        proc = subprocess.run(
            [str(binary), "search", query, "--catalog", str(catalog)],
            env=env, text=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, timeout=15,
        )
...
def main() -> int:
    ...
    if args.tier2:
        t0 = time.perf_counter()
        _ = time_search_loop(binary, catalog, "warmup-probe", iters=1)
    ...
    clip_t2i = None
    if args.tier2:
        semantic = time_search_loop(binary, catalog, "assignment unit 00", iters=max(5, args.search_iters // 2))
        clip_t2i = time_search_loop(binary, catalog, "photo of a document", iters=max(5, args.search_iters // 2))
    ...
    ok = all(x["returncode"] == 0 for x in (cold, warm, one_change, dupes))
