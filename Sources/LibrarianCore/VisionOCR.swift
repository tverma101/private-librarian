...
public struct VisionOCR: Sendable {
    ...
    /// yields no text.
    public func recognizeScannedPDF(at path: String, broker: SourceBroker,
                                    pdfText: String?, maxBytes: Int64? = nil) -> Result? {
        ...
        guard let data = try? broker.completeSnapshot(path, maxBytes: maxBytes), !data.isEmpty else { return nil }
