...
/// Swappable decode contract for the media lane. Implementations consume a
/// complete broker-owned snapshot only; they never receive a source path or
/// authority over the source filesystem.
...
public struct WhisperCLITranscriptionProvider: SpeechTranscriptionProvider {
    ...
    public func transcribe(_ chunks: [PCMChunk]) -> [TranscriptSegment]? {
        ...
        do {
            ...
            process.arguments = ["-m", modelPath, "-f", input.path, "-oj", "-of", outputBase.path,
                                 "-np", "-l", "en"]
            // whisper writes its JSON to outputBase. Discard stdio so a noisy
            // local binary cannot fill an unread pipe and deadlock indexing.
            ...
            return rows.compactMap { row in
                ...
                guard start.isFinite, end.isFinite, start >= 0, end >= 0 else { return nil }
    ...
    private static func makeWAV(chunks: [PCMChunk]) throws -> Data {
        guard let first = chunks.first, first.sampleRate.isFinite, first.sampleRate > 0,
              first.sampleRate <= Double(UInt32.max) / 2, first.channels == 1,
              chunks.allSatisfy({
                  $0.sampleRate.isFinite && $0.sampleRate == first.sampleRate
                      && $0.channels == 1 && $0.samples.allSatisfy(\.isFinite)
              }) else {
        ...
        guard pcm.count <= Int(UInt32.max) - 44 else {
            throw MediaDecoderError.invalidConfiguration
        }
...
public struct AudioProbe: Sendable {
    ...
    public static func probe(bytes: Data, fileExtension extHint: String? = nil, tagHint: String? = nil) -> MediaAnalysisDecision {
        ...
        let isMediaContainer = isAudioExt || isVideoExt || looksLikeAudioMagic(bytes)

        // Non-media container: unlikely
        if !isMediaContainer {
            // Also check magic signatures for common audio containers
            return MediaAnalysisDecision(speechLikelihood: .unlikely, shouldTranscribe: false,
                                         reasonCodes: ["probe:non-media-container"])
        }
...
#if canImport(AVFoundation)
    fileprivate final class DurationBox: @unchecked Sendable {
        private let lock = NSLock()
        private var value: CMTime?

        func store(_ value: CMTime?) {
            lock.lock()
            self.value = value
            lock.unlock()
        }
    ...
    /// AVAsset property loading is asynchronous on modern macOS. The public
    /// probes remain synchronous because they run inside the existing bounded
    /// worker lane, so bridge the property load with a short timeout.
    fileprivate static func loadDuration(at url: URL) -> CMTime? {
        guard #available(macOS 13.0, *) else { return nil }
        let result = DurationBox()
        let finished = DispatchSemaphore(value: 0)
        let task = Task.detached(priority: .utility) {
            defer { finished.signal() }
            let asset = AVURLAsset(
                url: url, options: [AVURLAssetPreferPreciseDurationAndTimingKey: false])
            result.store(try? await asset.load(.duration))
        }
    ...
    private static func probeDurationViaAVFoundation(bytes: Data, ext: String) -> TimeInterval? {
        ...
        do {
            ...
            guard let dur = loadDuration(at: tmpURL) else { return nil }
...
public struct VideoSampler: Sendable {
    ...
    private static func sampleFramesAVFoundation(bytes: Data, extHint: String?) -> Int {
        ...
        do {
            ...
            guard let dur = AudioProbe.loadDuration(at: tmpURL) else { return 0 }
