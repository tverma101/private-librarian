...
public struct SimilarityCluster: Sendable, Equatable {
    ...
    /// Weakest accepted edge score in the cluster, so the UI never presents
    /// the strongest link as the confidence of the entire component.
    public let confidence: Float
    /// Stable, provider-neutral explanation derived from the accepted edge
    /// signal (for example, "exact hash match" or "embedding similarity").
    ...
    public init(id: String, members: [String], representative: String,
                ...
                confidence: Float = 0, reason: String? = nil) {
        ...
        self.reason = reason ?? (relation == .nearDuplicate ? "near-duplicate similarity" : "semantic similarity")
    }
}
...
public struct ExactHashEdgeAdapter: SimilarityEdgeAdapter {
    ...
    private let buckets: [Data: [String]]?
    public init() { self.buckets = nil }
    /// Optional indexed form used by the integration path. Exact-hash
    /// neighborhoods are grouped once instead of scanning every node for
    /// every changed node.
    public init(nodes: [SimilarityNode]) {
        var grouped: [Data: [String]] = [:]
        for node in nodes {
            if let hash = node.exactHash { grouped[hash, default: []].append(node.id) }
        }
    ...
    public func edges(from node: SimilarityNode, candidates: [SimilarityNode]) -> [SimilarityEdge] {
        ...
        let matches = buckets?[hash].map { ids in
            ids.compactMap { id in candidates.first(where: { $0.id == id }) }
        } ?? candidates
        return matches.compactMap { other in
...
public struct SimilarityClustering: Sendable {
    ...
    public func clusters(nodes: [String], edges: [SimilarityEdge], minimumScore: Float,
        ...
        if relation == nil {
            // Keep near-duplicate and semantic connectivity separate. A
            // mixed component must not be relabeled as whichever relation has
            // more edges, because that would hide the meaning of its links.
            return SimilarityRelation.allCases
                .flatMap { clusters(nodes: uniqueNodes, edges: edges,
                                    minimumScore: minimumScore, relation: $0,
        ...
        return groups.values.filter { $0.count >= 2 }.map { members in
            let sorted = members.sorted(); let rel = relation!
            let memberSet = Set(sorted)
            let groupEdges = usable.filter {
                memberSet.contains($0.a) && memberSet.contains($0.b) && $0.relation == rel
            }
            ...
            return SimilarityCluster(id: id, members: sorted,
                                     ...
                                     confidence: groupEdges.map(\.score).min() ?? 0,
                                     reason: reason(for: groupEdges, relation: rel))
        }.sorted { $0.id < $1.id }
    }
    ...
    public func incrementalUpdate(nodes: [SimilarityNode], existingEdges: [SimilarityEdge],
        ...
        // Re-score each changed node against the complete live candidate set.
        // Passing only the changed nodes would silently drop changed-to-stable
        // edges and corrupt components after an incremental update.
        for node in live where affected.contains(node.id) {
            for adapter in adapters {
                retained += adapter.edges(from: node, candidates: live)
            }
        }
    ...
    private func reason(for edges: [SimilarityEdge], relation: SimilarityRelation) -> String {
        guard let signal = edges.sorted(by: {
            ($0.score, $0.signal.rawValue) > ($1.score, $1.signal.rawValue)
        }).first?.signal else {
            return relation == .nearDuplicate ? "near-duplicate similarity" : "semantic similarity"
        }
        switch signal {
        case .exactHash: return "exact hash match"
        case .featurePrint: return "Vision feature-print similarity"
